...
def init_structural_encoding(gs, rw_dim=16, dg_dim=16):
    '''
    gs: 传进来的data_list, [graph1的data, graph2的data, graph3的data,...]
    '''

    for g in gs:
        A = to_scipy_sparse_matrix(g.edge_index, num_nodes=g.num_nodes)         #### scipy.sparse._coo.coo_matrix, (dim1, dim1)
        D = (degree(g.edge_index[0], num_nodes=g.num_nodes) ** -1.0).numpy()    #### numpy.ndarray, (dim1, )

        Dinv = sp.diags(D)                                                      #### scipy.sparse._dia.dia_matrix, (dim1, dim1)
        RW = A * Dinv                                                           #### scipy.sparse._csr.csr_matrix, (dim1, dim1)
        M = RW

        RWSE = [torch.from_numpy(M.diagonal()).float()]                         #### 获取主对角线的元素。对应论文中公式（1）
        M_power = M
        for _ in range(rw_dim-1):
            M_power = M_power * M
            RWSE.append(torch.from_numpy(M_power.diagonal()).float())
        RWSE = torch.stack(RWSE, dim=-1)                                        #### 按照维度拼接。对应论文中公式（1）
        ...
        g['x_s'] = torch.cat([RWSE, DGSE], dim=1)                               #### 拼接
...
#---------------------------------- 从TUDatase下载graph-level adnomaly detection的dataset，并进行切分 ------------------------------------#
def get_ad_split_TU(args, fold=5):
    DS = args.DS
    path = osp.join(osp.dirname(osp.realpath(__file__)), '.', 'data', DS)

    print('当前dataset是：{}'.format(DS))
    print('从TUDataset下载后存储的路径为：{}'.format(path))
    ...
    kfd = StratifiedKFold(n_splits=fold, random_state=0, shuffle=True)                  #### k折交叉，比KFold函数要更好。
...
def get_ad_dataset_TU(args, split, need_str_enc=True):
    '''
    split: ( array([idx0, idx1, idx2, idx3,...]), array([idx10, idx11, idx12,...]) )
    need_str_enc: need structural encoding, 需要结构编码
    '''
    ...
    if DS in ['IMDB-BINARY', 'REDDIT-BINARY', 'COLLAB']:
        dataset = TUDataset(path, name=DS, transform=(Constant(1, cat=False)))          #### 这3个dataset比较特殊
    else:
        dataset = TUDataset(path, name=DS)

    dataset_num_features = dataset.num_node_features                                    #### node特征的dimension

    data_list = []                                                                      #### 存放data
    label_list = []                                                                     #### 存放label

    for data in dataset:
        data.edge_attr = None                                                           #### 边的属性用不到
        data_list.append(data)
        label_list.append(data.y.item())

    if need_str_enc:
        data_list = init_structural_encoding(data_list, rw_dim=args.rw_dim, dg_dim=args.dg_dim) #### 进行structural encoding
    ...
    for data in data_train_:
        if data.y != 0:                                                                 #### 标签不为0，才用来进行trian。 label==0的为anomaly
            data_train.append(data)

    idx = 0
    for data in data_train:
        data.y = 0                                                                      #### data_train中，laebl都是非0的。此时都置为0
        data['idx'] = idx
        idx += 1                                                                        #### 重新从0进行ID编号

    for data in data_test:
        data.y = 1 if data.y == 0 else 0                                                #### label==0的为anomaly

    dataloader = DataLoader(data_train, batch_size=args.batch_size, shuffle=True)       #### torch_geometric.loader.DataLoader
    dataloader_test = DataLoader(data_test, batch_size=args.batch_size_test, shuffle=True)  #### torch_geometric.loader.DataLoader
    meta = {'num_feat':dataset_num_features, 'num_train':len(data_train)}               #### node特征的dimension，data_train中data的数量
